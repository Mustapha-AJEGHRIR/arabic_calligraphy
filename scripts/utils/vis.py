...
def draw_strokes(
    data, factor=1, svg_filename="tmp/sample.svg", stroke_width=3, square=False, return_res=False, crop=True
):
...
def generate_words(file):
    word_drawings = []
    annot = get_annotation(file).strip()
    char_comps = preprocess(annot)
...
def get_annotation(json_path):
    return json_path.split("/")[-1].split("_")[0]
